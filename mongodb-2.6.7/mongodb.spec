...
Source0:        http://fastdl.mongodb.org/src/%{name}-%{version}.tar.gz
...
install -p -D -m 644 %{name}.logrotate %{buildroot}%{_sysconfdir}/logrotate.d/%{name}
install -p -D -m 644 %{name}.conf %{buildroot}%{_sysconfdir}/%{name}.conf
install -p -D -m 644 %{daemon}.sysconfig %{buildroot}%{_sysconfdir}/sysconfig/%{daemon}

%if 0%{?fedora} >= 15 || 0%{?rhel} >= 7
install -p -D -m 644 %{name}-tmpfile %{buildroot}%{_tmpfilesdir}/%{name}.conf
install -p -D -m 644 %{daemon}.service %{buildroot}%{_unitdir}/%{daemon}.service
%else
install -p -D -m 755 %{daemon}.init %{buildroot}%{_initddir}/%{daemon}
%endif
